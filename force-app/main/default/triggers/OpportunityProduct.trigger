trigger OpportunityProduct on OpportunityLineItem ( after insert, after update) {    
    if(Utility.runOppproducttrigger) {
        //new OpportunityProductTriggerHandler(Trigger.oldMap, Trigger.newMap).run();
        ...
        if(trigger.isAfter && (trigger.isInsert || trigger.isUpdate))
        {
           ...
           list<OpportunityLineItem> Opplinelist_child=new list<OpportunityLineItem>();
            ...
            for(OpportunityLineItem Oppyline:[select Product_Network_Type__c,Product_Sub_Type__c,Controller_Code__c,Product_Type_2__c,Product2.Merged_Product_Code__c,Product2.Product_Type_2__c,Product2.Controller_Code__c,Product2.Component_Code__c,OpportunityId,Product2.parent_product__c,Product2.Legacy_product__c,Product_Name_Dynamic__c,SBQQ__QuoteLine__c,SBQQ__QuoteLine__r.SBQQ__RequiredBy__c from OpportunityLineItem where Opportunityid in:OppIds and SBQQ__QuoteLine__r.SBQQ__Quote__r.SBQQ__Primary__c=true])
            {
                system.debug('inside---------------');
                ...
                Oppyline.Product_Network_Type__c = '';
                 if(Oppyline.Product2.parent_product__c==false)
                 {
                    ...
                    {               
                        ...
                        //oppyline.Merged_Product_Code_2__c=oppyline.Product2.Merged_Product_Code__c;
            ...
            if(Opplinelist_child.size()>0)
                {
                    Utility.runOppproducttrigger=false;
            ...
            map<OpportunityLineItem,string> Oppline_mergecode=new map<OpportunityLineItem,string>();
            map<OpportunityLineItem,list<OpportunityLineItem>> OpLine_OplineList=new map<OpportunityLineItem,list<OpportunityLineItem>>();
            
            for(OpportunityLineItem opline:[select Product_Network_Type__c,Controller_Code__c,Product_Sub_Type__c,Product2.Merged_Product_Code__c,Product2.Controller_Code__c,Product_Type_2__c,Product2.Product_Type_2__c,Product2.Component_Code__c,OpportunityId,Product2.parent_product__c,Product2.Legacy_product__c,Product_Name_Dynamic__c,SBQQ__QuoteLine__c,SBQQ__QuoteLine__r.SBQQ__RequiredBy__c from OpportunityLineItem where Opportunityid in:OppIds and SBQQ__QuoteLine__r.SBQQ__Quote__r.SBQQ__Primary__c=true])
            {           
                QoOppPro_map.put(opline.SBQQ__QuoteLine__c,opline);
                
                if(opline.Product2.Product_Type_2__c=='Capacity')
                        {
                            Oppline_mergecode.put(QoOppPro_map.get(opline.SBQQ__QuoteLine__r.SBQQ__RequiredBy__c),opline.Product2.Component_Code__c);
                        }
            ...
            system.debug('merge---------'+Oppline_mergecode);
          ...
          //          system.debug('full map map----------'+Grand_OpLine_OplineList);
            ...
            {           
                ...
                if(opline.Product2.Legacy_product__c==true)
                { 
                    system.debug('merge------11');               
                    opline.Product_Network_Type__c='ISCSI';
                    //opline.Merged_Product_Code_2__c=opline.Product2.Merged_Product_Code__c;             
                ...
                {               
                    ...
                    if(FCcount>=1)
                    {
                        if(Oppline_mergecode.containskey(opline))
                        {
                            string COMP_Code='';
                            if(opline.Product2.Component_Code__c!=null)
                            COMP_Code=opline.Product2.Component_Code__c+'-';
                            //system.debug('Inside merge-----------------'+Oppline_mergecode.get(opline));
                            //opline.Merged_Product_Code_2__c=COMP_Code+Oppline_mergecode.get(opline);                    
                        }
                        else
                        {
                        //opline.Merged_Product_Code_2__c=opline.Product2.Merged_Product_Code__c;
