trigger RecommendationTrigger on Recommendation__c (before insert, before update, after update) {
    boolean activateTrigger = ActivateRecommendationTrigger__c.getInstance().Activate__c;
    if(activateTrigger == true){
        ...
        if(Trigger.isBefore)
        {
            ...
            List<Asset> assts = [select Id, SerialNumber, Lead__c, Lead__r.Status, Lead__r.IsConverted, Account.Name,Account.BillingCity,Account.BillingCountry,Account.BillingPostalCode,Account.BillingState,Account.BillingStreet,Account.phone, account.ownerid,Account.Owner.IsActive from Asset where SerialNumber in :asstToRecMap.keySet()];
            ...
            for(String asstNum : asstToRecMap.keySet())
            {
                ...
                if(recList.size() > 0)
                {
                    ...
                    {
                        /* START - If Account Owner is Inactive, Assign to infosight user '0053400000B1jMI'
                         * Modified Date - 26th Sept 2017
                         * Author - Exafort(Azar)
                         * */
                        Lead l;
                        if(ast.account.owner.IsActive == false){
                            ID infosightUser = RecTriggerOwner__c.getInstance().InfosightUserID__c;
                            l = new Lead(Lastname='InfoSight Lead'+'- '+ ast.SerialNumber,Asset__c =ast.Id,   company = ast.Account.Name, phone = ast.Account.phone,state =ast.account.billingstate,city=ast.Account.BillingCity,country =ast.account.billingcountry,postalcode=ast.account.billingpostalcode, RecordTypeId = LEAD_INFOSIGHT_RECORDTYPEID, ownerid=infosightUser, LeadSource ='InfoSight Recommendation Program');
                        }
                        else if(ast.account.owner.IsActive == true){
