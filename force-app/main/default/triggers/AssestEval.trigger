...
{
    //system.debug('before if condition'+at.Opportunity_Asset__c);
    //Date AssetCreatedDate = at.CreatedDate.date();
    if (at.Opportunity_Asset__c!=null)
    {
        system.debug('if it is in the if condition'+at.Opportunity_Asset__c);
    if (at.Order_Type__c=='Eval Unit')
    {  
    opportunity opp=[SELECT id, Next_Steps_Archive__c, Name, OwnerID FROM Opportunity where id=:at.Opportunity_Asset__c];
