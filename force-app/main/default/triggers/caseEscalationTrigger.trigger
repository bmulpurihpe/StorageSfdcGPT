   ...
   if (Trigger.isBefore)
     {for (caseEscalation__c recordToProcess : Trigger.new)
        {         
            if(Trigger.isInsert)
            {
                recordToProcess.caseEscalationPriorityCreate__c = recordToProcess.caseEscalationPriority__c;
            }
             
         
         if(recordToProcess.caseEscalationJIRAId__c != null)
         {
             if(!recordToProcess.caseEscalationJIRAId__c.contains('ESC-'))
                 recordToProcess.caseEscalationJIRAId__c = 'ESC-' + recordToProcess.caseEscalationJIRAId__c;
         }
         ...
         List <caseEscalation__c> caseEscalations = [SELECT Id
                                                     ...
                                                     WHERE  caseEscalationCase__c           = :recordToProcess.caseEscalationCase__c AND
                                                            caseEscalationTypeDefinition__c = :recordToProcess.caseEscalationTypeDefinition__c AND
                                                            Name <> :recordToProcess.Name];
   ...
   // on the Case with the child escalation types on the Case.
     ...
     {for (caseEscalation__c recordToProcess : Trigger.isDelete ? Trigger.old : Trigger.new)
        {// Retrieve the list of all Case Escalation Type Definition Groups for this same case.
         ...
         if(!aCase.caseEscalationType__c.contains('Management Escalation'))
              aCase.caseEscalationType__c = null;
          else{
              aCase.caseEscalationType__c = 'Management Escalation';
          }
         ...
         //MW:TS-10277:Added lines to set Case IsEscalation field to true or false
         if(aCase.caseEscalationType__c == null) {
              aCase.caseEscalationType__c = 'Not Escalated';
              aCase.IsEscalated = false;
          } else {
              aCase.IsEscalated = true;
          }
